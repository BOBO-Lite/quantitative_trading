...
STRATEGY_VERSION = "S1.1-execution-audit-v5-research"
BENCHMARK = "000905.SH"
CAPITAL = 36574.55
...
AUDIT_STAMP_TAX_SELL = 0.001  # 平台压力口径；冻结口径为0.0005。
...
def _size_for_entry(candidate, reference_price, equity, cash, current_exposure):
    adjusted_to_raw = candidate["signal_close_raw"] / candidate["signal_close_adjusted"]
    ...
    while qty >= 100 and (_buy_cash_required(reference_price, qty) > cash
                         or _planned_loss(reference_price, stop_price, qty) > risk_budget):
        qty -= 100
...
def _admissible_entry_cap(candidate):
    """4%是价格上界而非必须报出的价格；把委托价收紧到结构风险可接受范围。"""
    ratio = candidate['signal_close_raw'] / candidate['signal_close_adjusted']
    structure_low = candidate['structure_low10_adjusted'] * ratio
    cap = min(candidate['signal_close_raw'] * (1 + MAX_PRICE_ABOVE_SIGNAL_CLOSE),
              structure_low / (1 - MAX_STOP_DISTANCE))
    if not np.isfinite(cap) or cap <= 0:
        return None
    return math.floor((cap + 1e-10) * 100) / 100.


def _entry_rejected(reason):
    g.entry_rejections[reason] = g.entry_rejections.get(reason, 0) + 1


def _buy_cash_required(price, quantity):
    """限价买入预留最低佣金及独立过户费，不依赖平台合并费用近似。"""
    if quantity <= 0:
        return 0.0
    value = price * quantity
    return value + max(MINIMUM_COMMISSION, value * COMMISSION_RATE) + value * TRANSFER_FEE_EACH_SIDE


def _planned_loss(price, stop, quantity):
    """计划止损损失计入买入费、卖出滑点和压力税费；不保证跳空可成交。"""
    proceeds = stop * (1 - SLIPPAGE_ROUND_TRIP / 2) * quantity
    sell_fee = max(MINIMUM_COMMISSION, proceeds * COMMISSION_RATE)
    sell_fee += proceeds * (TRANSFER_FEE_EACH_SIDE + AUDIT_STAMP_TAX_SELL)
    return _buy_cash_required(price, quantity) - proceeds + sell_fee


def _accumulate_session_bar(state, now, open_price, volume, turnover):
    """从09:31真实首分钟累计量额；09:30开盘增强bar可能仍是昨日数据。"""
    if (now.hour, now.minute) < (9, 31):
        return state
    stamp = now.strftime('%Y%m%d%H%M')
    if state is None:
        if (now.hour, now.minute) != (9, 31):
            return None  # 错过首分钟不能用09:45开盘冒充日开盘。
        state = {'open': float(open_price), 'volume': 0., 'turnover': 0., 'last': None}
    if state['last'] == stamp:
        return state
    if not all(np.isfinite(v) for v in (open_price, volume, turnover)) or min(volume, turnover) < 0:
        return None
    if state['last'] is not None:
        prior = pd.Timestamp(state['last'])
        if (pd.Timestamp(now) - prior).total_seconds() != 60:
            return None  # 入场窗口仅上午；缺失分钟失败关闭。
    result = dict(state)
    result['last'] = stamp
...
def _breakeven_price(entry_price, quantity):
    """求覆盖两边最低佣金、独立过户费和卖出税的成交价。"""
    if quantity <= 0 or entry_price <= 0:
        raise ValueError("保本价需要正的价格和数量")
    buy_value = entry_price * quantity
    buy_fee = max(MINIMUM_COMMISSION, buy_value * COMMISSION_RATE)
    buy_fee += buy_value * TRANSFER_FEE_EACH_SIDE
    total_cost = buy_value + buy_fee
    proportional = total_cost / (
        quantity * (1 - COMMISSION_RATE - TRANSFER_FEE_EACH_SIDE - AUDIT_STAMP_TAX_SELL)
    )
    minimum = (total_cost + MINIMUM_COMMISSION) / (
        quantity * (1 - TRANSFER_FEE_EACH_SIDE - AUDIT_STAMP_TAX_SELL)
    )
    return max(proportional, minimum)


def _closing_ma10(previous_daily, today_close):
    """官方history日线不含当天：前9日原始收盘与今日收盘合成当日MA10。"""
    previous = pd.to_numeric(previous_daily['close'], errors='coerce').tail(9)
    if len(previous) != 9 or not np.isfinite(previous).all() or not np.isfinite(today_close):
        raise ValueError('MA10 requires nine previous closes and current close')
    if min(float(previous.min()), today_close) <= 0:
        raise ValueError('MA10 prices must be positive')
    return (float(previous.sum()) + today_close) / 10.
...
def before_trading(context):
    ...
    if not market_gate:
        # 市场关闭只阻止开仓，不能取消已有持仓的行情订阅。
        retained = set(g.positions_meta)
        for symbol in list(g.subscribed - retained):
            unsubscribe(symbol)
        for symbol in list(retained - g.subscribed):
            subscribe(symbol)
        g.subscribed = retained
        g.candidates = {}
        g.open_prices = {}
        g.entry_orders = {}
        g.entry_intents = {}
    ...
    selected = table.loc[table["signal"]] if not table.empty else table
    ...
    needed = set(candidates) | set(g.positions_meta)
    for symbol in list(g.subscribed - needed):
        unsubscribe(symbol)
    for symbol in list(needed - g.subscribed):
        subscribe(symbol)
    g.subscribed = needed
...
def handle_bar(context, bar_dict):
    now = get_datetime()
    if (now.hour, now.minute) < (9, 31):
        return  # 实测09:30可能返回昨日最后一根bar，不能触发今日开仓或止损。
    ...
    for symbol, meta in list(g.positions_meta.items()):
        ...
        if current is None or bool(current.is_paused):
            continue
        if symbol in g.pending_open_exit:
            _submit_exit(symbol, qty, "scheduled_exit_retry", now)
            continue
    ...
    # avg_price是单分钟均价；自行从09:31累计成交额/成交量。
    for symbol, bar in current_map.items():
        if bar is not None:
            flow = _accumulate_session_bar(g.minute_flow.get(symbol), now,
                                           float(bar.open), float(bar.volume), float(bar.turnover))
            g.minute_flow[symbol] = flow
            if flow is not None:
                g.open_prices[symbol] = flow['open']
    ...
    for symbol, candidate in sorted(g.candidates.items(), key=lambda item: item[1]["score"], reverse=True):
        ...
        if bar is None or bool(bar.is_st) or bool(bar.is_paused):
            continue
        flow = g.minute_flow.get(symbol)
        if flow is None or flow['volume'] <= 0 or symbol not in g.open_prices:
            _entry_rejected('minute_flow_missing')
            continue
        gap = g.open_prices[symbol] / candidate["signal_close_raw"] - 1.0
        if not OPEN_GAP_MIN <= gap <= OPEN_GAP_MAX:
            _entry_rejected('opening_gap')
            continue
        if float(bar.close) >= float(bar.high_limit):
            _entry_rejected('limit_up')
            continue
        if not (
            float(bar.close) > candidate["signal_high_raw"]
            and float(bar.close) > flow['turnover'] / flow['volume']
        ):
            _entry_rejected('signal_not_confirmed')
            continue
        max_price = _admissible_entry_cap(candidate)
        if max_price is None or float(bar.close) > max_price:
            _entry_rejected('price_exceeds_risk_cap')
            continue
        reference_price = max_price
        plan = _size_for_entry(candidate, reference_price, equity, cash, current_exposure)
        if plan is None:
            _entry_rejected('risk_or_cash_or_minimum_value')
            continue
        ...
        cash -= _buy_cash_required(max_price, plan["quantity"])
        position_count += 1
...
def after_trading(context):
    ...
    for symbol, meta in list(g.positions_meta.items()):
        if _position_quantity(positions, symbol) <= 0:
            ...
            continue
        today = get_datetime().strftime("%Y%m%d")
        if meta.get("last_counted_date") != today:
            # 买入日是第一个持有交易日；停牌不暂停持有期限。
            meta["holding_days"] += 1
            meta["last_counted_date"] = today
        if meta["holding_days"] >= MAX_HOLDING_DAYS:
            g.pending_open_exit.add(symbol)
        bar = current_map.get(symbol)
        if bar is None or bool(bar.is_paused):
            continue
        # T+1 禁止当日卖出，但不禁止用当日收盘更新下一交易日保护。
        close = float(bar.close)
        meta["highest_close"] = max(float(meta["highest_close"]), close)
        close_r = (close - meta["entry_price"]) / meta["initial_r"]
        if close_r >= BREAKEVEN_AT_R:
            qty = max(100, _position_quantity(positions, symbol))
            meta["effective_stop"] = max(
                meta["effective_stop"],
                _breakeven_price(meta["entry_price"], qty) / (1 - SLIPPAGE_ROUND_TRIP / 2),
            )
        if close_r >= TRAILING_AT_R:
            # 当前原始价与前复权序列不能直接混用；跨除权处理仍单独待验收。
            daily = history(symbol, ["close"], 9, "1d", False, None, True, False)
            ma10 = _closing_ma10(daily, close)
    ...
    log.info('ENTRY_REJECTIONS {} {}'.format(get_datetime(), g.entry_rejections))
