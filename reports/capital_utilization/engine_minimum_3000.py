def run_bundle(bundle, policy=Policy(), rules=None, minute_loader=None):
    ...
    for index, day in enumerate(days, 1):
        ...
        audit=dict(date=date,route=route,screen_rows=len(rows),selected=sum(bool(select_signal(r,route,policy)) for r in rows),minute_eligible=len(candidates),confirmed=0,size_rejected=0,portfolio_rejected=0,submitted=0)
        attempted.clear()
        flow = {}
        ...
        audit['flat_close']=not bool(broker.positions)
    ...
    return dict(utilization_audit=daily_audit,status='RESEARCH_PORTFOLIO_REPLAY_ONLY', formal_strategy_validated=False,
