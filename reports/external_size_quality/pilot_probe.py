...
def before_trading(context):
    ...
    members=set()
    for code in indexes:
        values=sorted(get_index_stocks(code,asof))
        emit('index_'+code,dict(code=code,asof=asof,members=values))
        members.update(values)
    members=sorted(s for s in members if (s.startswith(('000','001','002','003')) and s.endswith('.SZ')) or (s.startswith(('600','601','603','605')) and s.endswith('.SH')))
    univ=get_all_securities('stock',asof)
    emit('securities',packet(univ))
    emit('config',dict(asof=asof,members=members,quarters=['2016q4','2017q1','2017q2','2017q3'],purpose='complete_single_signal_pilot_not_performance'))
    for offset in range(0,len(members),200):
        ...
        emit('prices_'+str(offset),{s:packet(frames[s]) for s in batch})
        ...
        emit('valuation_'+str(offset),packet(get_fundamentals(q,date='20171229')))
        queries={
        ...
        for quarter in ['2016q4','2017q1','2017q2','2017q3']:
            for name,q in sorted(queries.items()):
                emit(name+'_'+quarter+'_'+str(offset),packet(get_fundamentals(q,statDate=quarter,latest=True)))
    log.info('SIZEQ_PILOT_DONE')
