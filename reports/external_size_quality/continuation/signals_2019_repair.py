...
def snapshot(asof,trade):
    ...
    retired={s:deadline for s,deadline in {'600270.SH':'2018-12-28','600401.SH':'2019-07-12'}.items() if s in universe and asof>=deadline}
    universe=[s for s in universe if s not in retired]
    meta=get_all_securities('stock',asof)
    if '000043.SZ' in universe and '000043.SZ' not in meta.index:
        oldmeta=get_all_securities('stock','2019-12-12')
        meta.loc['000043.SZ']=oldmeta.loc['000043.SZ']
    pool=[];fin=[]
    ...
    return dict(asof=asof,trade=trade,indexes=indexes,retired=retired,pool=pool,financial=fin,fetched=fetched,decision=decision)
...
def before_trading(context):
    ...
    repair_dates=['2019-01-03', '2019-01-10', '2019-01-17', '2019-07-18', '2019-07-25', '2019-08-01', '2019-08-08', '2019-08-15', '2019-08-22', '2019-08-29', '2019-09-05', '2019-12-13']
    schedule=[(s,t) for s,t in schedule if s in repair_dates]
    emit('schedule',dict(repair_dates=repair_dates,year=YEAR,pilot=PILOT,all_days=days,schedule=schedule,interval=5,execution_not_simulated=True))
