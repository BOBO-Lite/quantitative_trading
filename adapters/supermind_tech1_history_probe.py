    ...
    for offset in range(0,len(ordered),100):
        ...
        for symbol in ordered[offset:offset+100]:
            ...
            for day in dates:
                ...
                bm_values=out['benchmark']
                if ONLY_ENTRY_DAYS and not bm_values['close']>bm_values['ma20']>bm_values['ma60']:
                    out['excluded'].setdefault('MARKET_CLOSED',[]).append(symbol);continue
                ...
                if count<120:reason='SHORT_HISTORY'
                    ...
                    if reason is None and ONLY_EXECUTABLE:
                        cap=int((r['raw_close']*1.04+1e-10)*100)/100
                        if max(2*r['atr20_raw']/cap,.03)>.06:reason='RISK_DISTANCE'
                    if reason is None:
