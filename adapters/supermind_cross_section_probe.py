...
def init(context):
    ...
    log.info('CROSS_SECTION_READ_ONLY_V2; NO ORDERS')
...
def before_trading(context):
    ...
    for symbol in symbols:
        ...
        if frame is None or frame.empty:
            ...
            row['observed_bars'] = int((np.isfinite(close) & (close > 0)).sum())
            if row['observed_bars'] < 120:
                row['reason'] = 'SHORT_LISTING_HISTORY'
            elif list(frame.index[-61:]) != list(benchmark.index) or not np.isfinite(close[-61:]).all() or min(close[-61:]) <= 0:
