...
def main():
    symbol='002317.SZ';case=json.loads((OUT/'case_002317.json').read_text(encoding='utf8'));buy=case['fills']['E1'][0]
    ...
    for mode in ('E1','W','P'):
        ...
        for day in case['daily']:
            ...
            for bar in cache.minutes[symbol,date]:
                t=bar['datetime']
    ...
    save('case_exit_repair.json',dict(scope='Original entry time/price/quantity fixed. Individual trade only; drawdown=0 because original case never reached hard drawdown while held. No cash dividends in the constant-factor window; original E1/W fills exactly reproduced.',results=output))
