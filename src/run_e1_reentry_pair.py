...
from e1_confirmation import ConfirmationRules,ConfirmationLoader
from e1_reentry_pair import ReentryRules,MODES
from report_e1_asymmetric import decomposition

ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'reports/e1_reentry_pair'
...
def main():
    ...
    for folder in ('e1_rank_research','e1_conditions','e1_loss_signals','e1_asymmetric','e1_turning','e1_confirmation'):
        for p in sorted((ROOT/'reports'/folder).glob('minute_export_*.txt')):cache=merge_cache([cache,read_export(p)])
    loader=ConfirmationLoader(cache,bundle);loaded=set()
    ...
    parity=[]
    for name,rules,folder in [('R',ReentryRules('E1R'),ROOT/'reports/e1_asymmetric'),('F',ConfirmationRules('F'),ROOT/'reports/e1_confirmation')]:
    ...
    while len(statuses)<len(wanted):
        ...
        for mode,cost in wanted:
            ...
            rules=ReentryRules(mode)
            try:
                result=run_bundle(bundle,Policy(cost_multiplier=cost),rules=rules,minute_loader=loader.load)
                save(f'{mode}_cost{cost}.json',result);save(f'{mode}_cost{cost}_events.json',[dict(e,date=bundle['calendar'][e['day_index']]) if 'day_index' in e else e for e in rules.events])
